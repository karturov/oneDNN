...
status_t stream_t::init() {
    ...
    if (is_profiling_enabled() || is_verbose_profiler_enabled()) {
        ...
        if (is_verbose_profiler_enabled()) {
            verbose_profiler_ = utils::make_unique<xpu::ze::verbose_profiler_t>(
                    this, 1e9 / device_properties.timerResolution,
                    max_timestamp_value);
        }
    }
...
status_t stream_t::run_verbose_profiler(
    ...
    CHECK(verbose_profiler_->add_to_pending_primitive_list(start_ms, pd_info));
    return status::success;
}
