...
struct ref_grouped_t : public primitive_t {
    ...
    struct pd_t : public matmul::pd_t {
        ...
        status_t init(impl::engine_t *engine) {
            ...
            const bool matching_dt = src_dt_ == wei_dt_
                    && utils::one_of(
                            src_dt_, f32, bf16, f16, f8_e5m2, f8_e4m3, f4_e2m1);
            ...
            if (!attr_scales.has_default_values(DNNL_ARG_SRC)) {
                ...
                const int grouped_mask = src_qmask_M() | src_qmask_K();
                // Row-wise or grouped scales supported for src
                VDISPATCH_MATMUL(
                        utils::one_of(src_mask, rowwise_mask, grouped_mask),
                        VERBOSE_UNSUPPORTED_SCALES_CFG);
                // f32, f16, or f8_e4m3 scale data types
                VDISPATCH_MATMUL(
            ...
            // Store src scale group K size
            if (!attr_scales.has_default_values(DNNL_ARG_SRC)
                    && !attr_scales.get(DNNL_ARG_SRC).has_default_groups()) {
                src_scale_group_k_ = attr_scales.get(DNNL_ARG_SRC).get_group(1);
            }
    ...
    status_t init(impl::engine_t *engine) override {
        ...
        // Src scale group size for grouped quantization
        kernel_ctx.define_int(
                "SRC_SCALE_GROUP_K", (int)pd()->src_scale_group_k_);
