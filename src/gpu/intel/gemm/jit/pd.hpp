...
struct init_view_t {
    ...
    // Not serialized (init_view_t never is), so it cannot perturb cache keys;
    // copied with view_ on clone, so a cloned pd still passes the assert.
...
// pd_t + exec_ctx_t state in kernel-view orientation: all A/B-side fields are
...
struct exec_view_t : init_view_t {
    ...
    // alpha, with_mx_scale and cmask are inherited from init_view_t: the Phase-A
    // snapshot is the kernel-selection input; build_exec_view overwrites alpha
    ...
    // The quant mds are already swap-folded on the view, so these forward to
