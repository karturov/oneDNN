...
// the GEMM-relevant types, so this round-trips exactly. Used by init_view_t's
...
// (pd_t::init_attrs / init_post_ops) write into this view in user
...
struct init_view_t {
    ...
    // Bias config (swap-invariant; bias is fused as the C-side
    // offset/binary input).
...
struct pd_t : public gemm::pd_t {
    ...
    status_t init_attrs(init_view_t &cfg, impl::engine_t *engine);
    status_t init_post_ops(init_view_t &cfg, impl::engine_t *engine);
    ...
    // xe_hp_systolic.
    status_t seed_problem(init_view_t &cfg);

    // Phase A validators — read cfg + attr; report errors in user terms.
    status_t scales_ok(const init_view_t &cfg, impl::engine_t *engine);
    status_t zp_ok(const init_view_t &cfg, impl::engine_t *engine);
    status_t gs_ok(const init_view_t &cfg, impl::engine_t *engine);
    ...
    virtual bool decide_swap_ab(const init_view_t &cfg) const { return false; }
    ...
    init_view_t view_;
    const init_view_t &view() const { return view_; }
    ...
    bool swap_ab() const { return view_.swap_ab; }
    ...
    // view_.{a,b}_*_md. Contrast stride() above, which reads the unswapped
    ...
    // (kernel-oriented) leading dim and type taken from the view; `batch_arg`
...
// Runtime-time projection of pd_t + exec_ctx_t state in kernel-view
// orientation. Inherits init_view_t's swap-folded scalars/flags and adds
...
// transfer_post_ops) take 'const init_view_t&' and so cannot reach any of
// the runtime fields below — the type still enforces phase separation.
struct exec_view_t : init_view_t {
    ...
    // Base offsets, in elements (kernel-view: a is the kernel's A storage).
    size_t off_a0 = 0;
    size_t off_b0 = 0;
    size_t off_c0 = 0;
    int64_t off_co0 = 0;
    ...
    // on the view and are already swap-folded to kernel orientation by
