...
struct pd_t : public gemm::pd_t {
    ...
    kernel_config_t config_;
    const kernel_config_t &config() const { return config_; }
    ...
    bool swap_ab() const { return config_.swap_ab; }
...
// alpha); the finalized config_ is reached by reference via cfg(). A/B storage
// is swap_ab-routed once in build_exec_config; the config it reaches is folded.
struct exec_config_t {
    const pd_t *pd = nullptr;
    const kernel_config_t &cfg() const { return pd->config(); }
