...
// swap_ab routing: kernel-A is fed by user-B when swapped, applied once per
// ctx-arg binding instead of a post-hoc std::swap.
template <typename T>
static T pick_a(bool s, T ua, T ub) {
    return s ? ub : ua;
}
template <typename T>
static T pick_b(bool s, T ua, T ub) {
    return s ? ua : ub;
}
