...
status_t xe_hp_systolic_t::pd_t::init(impl::engine_t *engine) {
    ...
    // orientation, post-op binary metadata) into view_, shared with gen_t.
    ...
    // view_ stays in (kernel == user) orientation. pad_lda is intentionally
...
status_t xe_hp_systolic_t::init_compute(impl::engine_t *engine) {
    ...
    // Build the canonical problem from the shared view/problem init path
    ...
    GEMMProblem base;
    CHECK(pd()->init_GEMMProblem(base, intel_engine, pd()->view()));
    problem_ = base;
