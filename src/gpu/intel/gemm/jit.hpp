    ...
    struct pd_t : public jit::pd_t {
        ...
        bool decide_swap_ab(const jit::init_view_t &cfg) const override {
            ...
            bool check_lda = ((d->transa() == dnnl_notrans && d->lda() == 1)
                    || (d->transa() == dnnl_trans));
            bool s = (d->m() == 1 && d->ldc() == 1 && check_lda)
                    || d->transc() == dnnl_trans;
        ...
        status_t init(impl::engine_t *engine) {
            ...
            // Phase A — populate view_ in user orientation and validate
            // user attrs.
            ...
            // Seed the view's user-orientation A/B/C scalars. Phase B
            // (pad_lda + swap_fold) reads/mutates these in place.
            view_.a_type = d->a_type();
            view_.b_type = d->b_type();
            view_.c_type = d->c_type();
            view_.m = d->m();
            view_.n = d->n();
            view_.k = d->k();
            view_.lda = lda_;
            view_.ldb = ldb_;
            view_.ldc = d->ldc();
            view_.trans_a = transa_;
            view_.trans_b = transb_;
            view_.sum_ab = d->sum_ab;
            view_.trans_co = (d->trans_bias() == dnnl_trans);

            // Phase B — decide swap, pad lda/ldb, fold into kernel
            // orientation. After this point view_ is the kernel projection.
            ...
            lda_ = view_.lda;
            ldb_ = view_.ldb;
            transa_ = view_.trans_a;
            transb_ = view_.trans_b;
            jit::swap_fold(view_, swap);
            // Mirror view_ alignments so pd_t::align() (which still reads
            // user-orientation ld(arg)) feeds init_GEMMProblem consistently.
            view_.align_a = align(swap ? DNNL_ARG_B : DNNL_ARG_A);
            view_.align_b = align(swap ? DNNL_ARG_A : DNNL_ARG_B);
            view_.align_c = align(DNNL_ARG_C);
            ...
            // int/uint32_t
            VDISPATCH_GEMM(std::max({cfg.m, cfg.n, cfg.k, d->batch()})
