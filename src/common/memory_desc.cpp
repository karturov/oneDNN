...
status_t memory_desc_init_with_grouped_encoding(memory_desc_t &memory_desc,
    ...
    // inner_* are zero initialized,
    // since grouped encoding does not support nested blocking
    auto &vd = grouped.values_desc;
    vd.strides[variable_dim_idx] = dims[1 - variable_dim_idx];
    vd.strides[1 - variable_dim_idx] = 1;
