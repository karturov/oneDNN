...
// clang-format off
#define BRGEMM_FP8_FWD_CONVS(dtsrc, dtwei) { \
    {forward, dtsrc, dtwei, data_type::undef}, { \
...
#define BRGEMM_FP8_BWD_D_CONVS(dtwei, dtdst) { \
    {backward_data, data_type::undef, dtwei, dtdst}, REG_BWD_D_PK({ \
...
#define BRGEMM_FP8_BWD_W_CONVS(dtsrc, dtdst) { \
    {backward_weights, dtsrc, data_type::undef, dtdst}, REG_BWD_PK({ \
...
const std::map<pk_dt_impl_key_t, std::vector<impl_list_item_t>> &impl_list_map() {
    static const std::map<pk_dt_impl_key_t, std::vector<impl_list_item_t>> the_map = REG_CONV_P({
        ...
        {{forward, f32, f16, data_type::undef}, {
            CPU_INSTANCE_AVX512(brdgmm_dw_convolution_fwd_t)
        ...
        BRGEMM_FP8_FWD_CONVS(f8_e5m2, f8_e5m2),
        BRGEMM_FP8_FWD_CONVS(f8_e5m2, f8_e4m3),
        BRGEMM_FP8_FWD_CONVS(f8_e4m3, f8_e5m2),
        BRGEMM_FP8_FWD_CONVS(f8_e4m3, f8_e4m3),
        // BWD_D fp
        {{backward_data, data_type::undef, f32, f32}, REG_BWD_D_PK({
        ...
        {{backward_data, data_type::undef, bf16, f32}, REG_BWD_D_PK({
            CPU_INSTANCE_AVX512(brgemm_convolution_bwd_t<avx512_core>)
            CPU_INSTANCE_AVX512(brgemm_convolution_bwd_strided_t<avx512_core>)
            CPU_INSTANCE_AVX2(brgemm_convolution_bwd_t<avx2>)
        ...
        {{backward_data, data_type::undef, f16, f32}, REG_BWD_D_PK({
            CPU_INSTANCE_AVX512(brgemm_convolution_bwd_t<avx512_core>)
            CPU_INSTANCE_AVX512(brgemm_convolution_bwd_strided_t<avx512_core>)
            CPU_INSTANCE_AVX2(brgemm_convolution_bwd_t<avx2>)
            CPU_INSTANCE_AVX512(brgemm_convolution_bwd_strided_t<avx2>)
        ...
        {{backward_data, data_type::undef, bf16, bf16}, REG_BWD_D_PK({
            CPU_INSTANCE_X64(ip_convolution_bwd_data_t)
            CPU_INSTANCE_AMX(brgemm_convolution_bwd_t<avx512_core_amx>)
        ...
        {{backward_data, data_type::undef, f16, f16}, REG_BWD_D_PK({
            CPU_INSTANCE_X64(ip_convolution_bwd_data_t)
        ...
        BRGEMM_FP8_BWD_D_CONVS(f8_e5m2, f8_e5m2),
        BRGEMM_FP8_BWD_D_CONVS(f8_e5m2, f8_e4m3),
        BRGEMM_FP8_BWD_D_CONVS(f8_e4m3, f8_e5m2),
        BRGEMM_FP8_BWD_D_CONVS(f8_e4m3, f8_e4m3),
        // BWD_W fp
        ...
        BRGEMM_FP8_BWD_W_CONVS(f8_e5m2, f8_e5m2),
        BRGEMM_FP8_BWD_W_CONVS(f8_e5m2, f8_e4m3),
        BRGEMM_FP8_BWD_W_CONVS(f8_e4m3, f8_e5m2),
        BRGEMM_FP8_BWD_W_CONVS(f8_e4m3, f8_e4m3),
        // FWD int8 (src:s8)
        ...
        {{backward_data, data_type::undef, s8, u8}, REG_BWD_D_PK({
            CPU_INSTANCE_AMX(brgemm_convolution_bwd_strided_t<avx512_core_amx>)
        ...
        {{backward_data, data_type::undef, s8, s8}, REG_BWD_D_PK({
            CPU_INSTANCE_AMX(brgemm_convolution_bwd_strided_t<avx512_core_amx>)
            CPU_INSTANCE_AVX512(brgemm_convolution_bwd_strided_t<avx512_core_vnni>)
            CPU_INSTANCE_AVX512(brgemm_convolution_bwd_strided_t<avx512_core>)
            CPU_INSTANCE_AVX2(brgemm_convolution_bwd_strided_t<avx2_vnni_2>)
...
const impl_list_item_t *get_convolution_impl_list(
    ...
    // Data types that can be different for a specific prop_kind are passed as
    // data_type::undef to match the shrunk list of signatured implementations.
    const auto src_dt = desc->prop_kind == backward_data
            ? data_type::undef
            : conv_prop_invariant_src_d(desc)->data_type;
    const auto wei_dt = desc->prop_kind == backward_weights
            ? data_type::undef
            : conv_prop_invariant_wei_d(desc)->data_type;
    const auto dst_dt = is_fwd ? data_type::undef
