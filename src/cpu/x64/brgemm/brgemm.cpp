...
status_t brgemm_desc_set_postops(brgemm_desc_t *brg,
    ...
    if (!IMPLICATION(one_of(data_type::f8_e5m2, dt_bias, dt_d)
                ...
                utils::one_of(
                        true, mayiuse(avx512_core_amx), mayiuse(avx10_2))))
        return status::unimplemented;
...
status_t brgemm_desc_set_attr(
    ...
    if (brg->is_fp8
        ...
        return status::unimplemented;
    if (brg->is_bf16_fp8 && !is_superset(brg->isa_impl, avx512_core_amx))
        return status::unimplemented;
    if (brg->is_f16_fp8 && !is_superset(brg->isa_impl, avx512_core_amx_fp16))
        return status::unimplemented;
