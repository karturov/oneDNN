...
Intel Graphics is supported by oneDNN GPU engine. GPU engine is disabled in the default build configuration.
...
Building oneDNN with OpenCL runtime requires an OpenCL SDK. You can explicitly specify the path to the SDK using CMake option ``-DOPENCLROOT``.
